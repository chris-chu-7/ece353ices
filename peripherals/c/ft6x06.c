...
{ 
  ...
  // This will require reading P1_XH and P1_XL
	...
	// Read from the register containing the upper byte
	ft6x06_set_addr(FT6X06_I2C_BASE, FT6X06_P1_XH_R);
	ft6x06_read_data(FT6X06_I2C_BASE, &byte);
	
	// Clear all bits except for [3:0] and shift up
	data = (byte & 0x0F) << 8;
	
	// Read from the register containing the lower byte
	ft6x06_set_addr(FT6X06_I2C_BASE, FT6X06_P1_XL_R);
	ft6x06_read_data(FT6X06_I2C_BASE, &byte);
	data |= byte;
	
	// Flip the x vals so 0 maps to the left edge of the screen
	return 239 - data;
} 
